...
class Cybernetikz_Events_AdminController extends Mage_Adminhtml_Controller_Action
{
	public function indexAction()
    {
		$this->loadLayout()
			->_addContent($this->getLayout()->createBlock('events/admin_main'))
			->renderLayout();
    }
	...
	public function deleteAction()
    {
        ...
        try {
            Mage::getModel('events/tip')->setId($tipId)->delete();
            Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('events')->__('Event successfully deleted'));
    ...
    {
        $this->loadLayout()
        ->_addContent($this->getLayout()->createBlock('events/admin_new'))
        ->renderLayout();
    }
        ...
        if ($data = $this->getRequest()->getPost()) {
        	...
        	if(isset($_FILES['main_file_url']['name']) && $_FILES['main_file_url']['name'] != '') {
				try {	
    	            ...
    	            $this->_getSession()->addException($e, Mage::helper('events')->__('Error uploading file. Please try again later.'));
		        }
            ...
            $tip = Mage::getModel('events/tip')->setData($data);
            try {
                $tip->save();
    ...
    {
        $this->loadLayout();
        $this->_addContent($this->getLayout()->createBlock('events/admin_edit'));
        $this->renderLayout();
    }
    ...
    public function saveAction()
    {
        ...
        if ($data = $this->getRequest()->getPost()) {
			...
			$tip = Mage::getModel('events/tip')->load($tipId)->addData($data);
            try {
                $tip->setId($tipId)->save();
 
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('events')->__('Event successfully updated'));
                $this->getResponse()->setRedirect($this->getUrl('*/*/'));
                return;
            } catch (Exception $e){
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
